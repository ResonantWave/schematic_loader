...
class App:
   def __init__(self, master):
      ...
      master.wm_title('Cargador de modelos')
      ...
      self.button = ttk.Button(frame, text = 'Cargar modelo', command = self.openfile)
      self.button.grid(row = 2, column = 2, padx = 5, pady = 5)

      self.button = ttk.Button(frame, text = 'Aplanar mapa', command = self.flatland)
      self.button.grid(row = 2, column = 1, padx = 5, pady = 5)

      self.button = tk.Button(frame, text = 'Borrar modelo', command = self.delete, fg = 'red')
      self.button.grid(row = 2, column = 0, padx = 5, pady = 5)

      self.button = ttk.Button(frame, text = 'Teletransportarse a 0,0,0', command = self.teletozero)
...
def loadSchematic(schematicToImport, deleteSchematic = 0, delX = 0, delY = 0, delZ = 0):
   ...
   if(infile['Width'] == "Classic"):
      raise ImportError('Este formato de schematic no está soportado')
   ...
   if(delete == False):
      ...
      mc.postToChat('Cargando {}...'.format(schematicToImport))
            ...
            for z in range(length):
               ...
               try:
                  if(((float(index) / len(infile['Blocks'])) * 100) % 10 == 0):
                     mc.postToChat('{}% cargado...'.format(str(index * 100 / len(infile['Blocks']))))
               except ZeroDivisionError:
                  continue

      mc.postToChat('Modelo cargado!')
   else:
      mc.postToChat('Borrando {}...'.format(schematicToImport))
      mc.setBlocks(delX,
		   delY,
		   delZ,
		   delX + width,
...
print('Esperando a que Minecraft se inicie...')
while 1:
   try:
